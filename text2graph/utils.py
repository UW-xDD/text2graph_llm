...
def get_eta(eval_db: Path, test_set: Path, run_name: str, n_workers: int) -> str:
    ...
    samples = cursor.execute(
        f"SELECT output FROM {run_name} ORDER BY RANDOM() LIMIT 1000"
    ).fetchall()
    eval_durations = []
    for r in samples:
        output = r[0]
    ...
    expected_finish_time = datetime.datetime.now() + datetime.timedelta(
        hours=hours_left
    )

    return f"Completed: {completed}/{n} ({completed/n*100:.2f}%) Expected finish: {expected_finish_time.strftime('%Y-%m-%d %H:%M:%S')}"
